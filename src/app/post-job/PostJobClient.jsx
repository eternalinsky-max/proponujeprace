...
function PostJobPage({ user }) {
  ...
  async function getIdTokenSafe() {
    if (user && typeof user.getIdToken === "function") return user.getIdToken(true);
  ...
  return (
    <section className="mx-auto max-w-2xl rounded-xl bg-white p-6 shadow-soft">
      ...
      <form onSubmit={handleSubmit} className="grid gap-4">
        <input className="input" placeholder="Tytuł oferty" value={title} onChange={(e) => setTitle(e.target.value)} required />
        <textarea className="input" rows={5} placeholder="Opis oferty" value={description} onChange={(e) => setDescription(e.target.value)} />
        <input className="input" placeholder="Miasto (opcjonalnie)" value={city} onChange={(e) => setCity(e.target.value)} />
        <div className="grid grid-cols-2 gap-3">
          <input className="input" placeholder="Minimalne wynagrodzenie" value={salaryMin} onChange={(e) => setSalaryMin(e.target.value)} inputMode="numeric" />
          <input className="input" placeholder="Maksymalne wynagrodzenie" value={salaryMax} onChange={(e) => setSalaryMax(e.target.value)} inputMode="numeric" />
        </div>
